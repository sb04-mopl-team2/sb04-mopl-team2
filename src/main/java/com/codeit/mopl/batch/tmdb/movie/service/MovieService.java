    ...
    try {
      while (!currentDate.isAfter(endDate)) {
        ...
        TmdbDiscoverMovieResponse firstPageResponse =
            movieApiService.discoverContentFromDateWithResponse(currentDate, 1).block();
        ...
        for (int page = 2; page <= maxPages; page++) {
          movieApiService.discoverContentFromDate(currentDate, page).block();
          totalPages++;
