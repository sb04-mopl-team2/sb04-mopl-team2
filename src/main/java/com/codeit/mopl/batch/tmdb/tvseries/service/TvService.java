...
public class TvService {
  ...
  /**
   * TV 프로그램 초기 데이터 로드 (6개월치)
   */
  public void runInitialDataLoad() {
    LocalDate endDate = LocalDate.now();
    LocalDate startDate = endDate.minusMonths(6);

    log.info("=== TV 프로그램 초기 데이터 수집 시작 ===");
    ...
    try {
      while (!currentDate.isAfter(endDate)) {
        ...
        TmdbDiscoverTvResponse firstPageResponse =
            tvApiService.discoverContentFromDateWithResponse(currentDate, 1).block();
        ...
        for (int page = 2; page <= maxPages; page++) {
          tvApiService.discoverContentFromDate(currentDate, page).block();
          totalPages++;
      ...
      log.info("=== TV 프로그램 초기 데이터 수집 완료 ===");
      log.info("총 처리 페이지 수: {}", totalPages);

    } catch (Exception e) {
      log.error("TV 프로그램 초기 데이터 수집 실패", e);
